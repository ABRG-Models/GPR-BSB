...
/*
   ...
   Node name: D1 MSN to D1 MSN Synapse 0 postsynapse
*/


#define COMPONENT_CLASS_STRING "dev/SpineML/temp/PS/GABAbuf"
#define COMPONENT_CLASS_CPP dev_spineml_ps_GABAbuf_0
...
//TimeDerivative
float (COMPONENT_CLASS_CPP::*  CurrentP__Pg_gaba)(float, int);
...
#define GABAbufX__XCurrent 1


// Global variables
vector < int > GABAbufO__Oregime;
vector < int > GABAbufO__OregimeNext;
...
numeric::Output PORTI_gaba;
//if using an alias then create the output variable

vector < double > I_gaba;

vector < double > I_gabaLOGVAR;
vector < int > I_gabaLOGMAP;
FILE * I_gabaLOGFILE;

numeric::Output PORTg_gaba;
//if using an alias then create the output variable

vector < double > g_gabaLOGVAR;
vector < int > g_gabaLOGMAP;
FILE * g_gabaLOGFILE;
...
// State Variables
	...
	vector <  double > g_gaba;
	string g_gaba_BINARY_FILE_NAME;
	string g_gaba_BINARY_FILE_NAME_OUT;
...
// Parameters
	...
	vector < double > tau_gaba;

	vector < double > E_gaba;
...
//TimeDerivative
float CurrentV__Vg_gaba(float val_BRAHMS, int num_BRAHMS) {
float return_val_BRAHMS;
g_gaba[num_BRAHMS] = val_BRAHMS;
return_val_BRAHMS = -g_gaba[num_BRAHMS]/tau_gaba[num_BRAHMS];
return return_val_BRAHMS;
}
//float (*  CurrentP__Pg_gaba)(float val_BRAHMS, int num_BRAHMS);
...
Symbol COMPONENT_CLASS_CPP::event(Event* event)
{
	switch(event->type)
	{
		case EVENT_STATE_SET:
		{
			...
			if (!(nodeState.hasField("g_gabaRANDXOVER2") || \
				nodeState.hasField("g_gabaOVER2") || \
				nodeState.hasField("g_gabaRANDX") || \
				nodeState.hasField("g_gaba") || \
				nodeState.hasField("g_gabaOVER1") || \
				nodeState.hasField("g_gabaBIN_FILE_NAME"))) {
				g_gaba.resize(numEl_BRAHMS, 0);
			}

			if (nodeState.hasField("g_gabaRANDXOVER2")) {
				g_gaba = nodeState.getField("g_gabaRANDXOVER2").getArrayDOUBLE();
				float val1_BRAHMS = 0;
				float val2_BRAHMS = 1;
				// if normal distribution
				if (g_gaba[0] == 1) {
					val1_BRAHMS = g_gaba[1];
					val2_BRAHMS = g_gaba[2];
					this->rngData_BRAHMS.seed = g_gaba[3];
					if (this->rngData_BRAHMS.seed == 0)
						this->rngData_BRAHMS.seed = getTime();
					g_gaba.resize(numEl_BRAHMS, 0);
					for (UINT32 i_BRAHMS = 0; i_BRAHMS < numEl_BRAHMS; ++i_BRAHMS) {
						g_gaba[i_BRAHMS] = (RNOR(&this->rngData_BRAHMS) * val2_BRAHMS) + val1_BRAHMS;
					}
				}
				...
				if (g_gaba[0] == 2) {
					...
					g_gaba.resize(numEl_BRAHMS, 0);
					for (UINT32 i_BRAHMS = 0; i_BRAHMS < numEl_BRAHMS; ++i_BRAHMS) {
						g_gaba[i_BRAHMS] = (UNI(&this->rngData_BRAHMS) * (val2_BRAHMS-val1_BRAHMS)) + val1_BRAHMS;
					}
				}
				finishedThis = true;
			}
			if (nodeState.hasField("g_gabaOVER2") && !finishedThis) {
				if (nodeState.getField("g_gabaOVER2").getDims()[0] == 1) {g_gaba = nodeState.getField("g_gabaOVER2").getArrayDOUBLE();
					if (g_gaba.size() == 1) {
						g_gaba.resize(numEl_BRAHMS, g_gaba[0]);
					} else if (g_gaba.size() != numEl_BRAHMS) {
						berr << "State Variable g_gaba has incorrect dimensions (Its size is " << g_gaba.size() << ", not " << numEl_BRAHMS << ")";
					}
					finishedThis = true;
				}
			}

			if (nodeState.hasField("g_gabaRANDX") && !finishedThis) {

				g_gaba = nodeState.getField("g_gabaRANDX").getArrayDOUBLE();
				float val1_BRAHMS = 0;
				float val2_BRAHMS = 1;
				// if normal distribution
				if (g_gaba[0] == 1) {
			...
			if (nodeState.hasField("g_gaba") && !finishedThis) {
			g_gaba = nodeState.getField("g_gaba").getArrayDOUBLE();
				if (g_gaba.size() == 1) {
					g_gaba.resize(numEl_BRAHMS, g_gaba[0]);
				} else if (g_gaba.size() != numEl_BRAHMS) {
					berr << "State Variable g_gaba has incorrect dimensions (Its size is " << g_gaba.size() << ", not " << numEl_BRAHMS << ")";
			...
			if (nodeState.hasField("g_gabaBIN_FILE_NAME") && !finishedThis) {
			        if (!nodeState.hasField("g_gabaBIN_NUM_ELEMENTS")) {
				        berr << "Found a binary file name without corresponding binary number of elements.";
				}
				g_gaba_BINARY_FILE_NAME = nodeState.getField("g_gabaBIN_FILE_NAME").getSTRING();
				int __temp_num_property_elements = nodeState.getField("g_gabaBIN_NUM_ELEMENTS").getINT32();

				// open the file for reading
				FILE * binfile;

				string fileName = modelDirectory_BRAHMS + "/" + g_gaba_BINARY_FILE_NAME;
				binfile = fopen(fileName.c_str(),"rb");
				if (!binfile) {
					berr << "Could not open properties list file: " << fileName;
				}

				g_gaba.resize(__temp_num_property_elements);
				...
				for (int i_BRAHMS = 0; i_BRAHMS < __temp_num_property_elements; ++i_BRAHMS) {
					    ...
					    berr << "Error loading binary state variable property g_gaba: index "
						 << __temp_property_list_indices[i_BRAHMS] << " out of range";
					}
					g_gaba[__temp_property_list_indices[i_BRAHMS]] = __temp_property_list_values[i_BRAHMS];
				}
			}


			if (nodeState.hasField("g_gabaOVER1") && !finishedThis)
			{
				vector < double > __tempInputValues__;
				__tempInputValues__ = nodeState.getField("g_gabaOVER1").getArrayDOUBLE();
				// since OVER1 always means sparse Values
				if (g_gaba.size() == 1) {g_gaba.resize(numEl_BRAHMS, g_gaba[0]);
				}
				if (g_gaba.size() != numEl_BRAHMS) {berr << "State Variable g_gaba has a ValueList override but no base Values";}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS < __tempInputValues__.size(); i_BRAHMS += 2) {
					if (__tempInputValues__[i_BRAHMS] > numEl_BRAHMS-1) {berr << "State Variable g_gaba has ValueList index " << float(i_BRAHMS)<< " (value of " << float(__tempInputValues__[i_BRAHMS]) << ") out of range";}
					g_gaba[__tempInputValues__[i_BRAHMS]] = __tempInputValues__[i_BRAHMS+1];
				}
			...
			{
				vector < double > __tempInputValues__;
				__tempInputValues__ = nodeState.getField("g_gabaOVER2").getArrayDOUBLE();
				// OVER2 here means sparse Values
				if (g_gaba.size() != numEl_BRAHMS) {berr << "Experiment State Variable g_gaba has a ValueList override but no base Values";}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS < __tempInputValues__.size(); i_BRAHMS += 2) {
					if (__tempInputValues__[i_BRAHMS] > numEl_BRAHMS-1) {berr << "Experiment State Variable g_gaba has ValueList index " << float(i_BRAHMS)<< " (value of " << float(__tempInputValues__[i_BRAHMS]) << ") out of range";}
					g_gaba[__tempInputValues__[i_BRAHMS]] = __tempInputValues__[i_BRAHMS+1];
				}
			...
			// output the values:
			/*bout << "g_gaba" << D_WARN;
			for (UINT32 i_BRAHMS = 0; i_BRAHMS < g_gaba.size(); ++i_BRAHMS) {
				bout << float(g_gaba[i_BRAHMS]) << D_WARN;
			}*/
			}
			...
			{
			bool finishedThis = false;
			if (nodeState.hasField("tau_gabaRANDXOVER2")) {
				this->allParamsDelaysAreFixedValue = false;
				this->tau_gaba = nodeState.getField("tau_gabaRANDXOVER2").getArrayDOUBLE();
				float val1_BRAHMS = 0;
				float val2_BRAHMS = 1;
				// if normal distribution
				if (this->tau_gaba[0] == 1) {
					val1_BRAHMS = this->tau_gaba[1];
					val2_BRAHMS = this->tau_gaba[2];
					this->rngData_BRAHMS.seed = tau_gaba[3];
					if (this->rngData_BRAHMS.seed == 0) {
						this->rngData_BRAHMS.seed = getTime();
					}
					this->tau_gaba.resize(numEl_BRAHMS, 0);
					for (UINT32 i_BRAHMS = 0; i_BRAHMS < numEl_BRAHMS; ++i_BRAHMS) {
						this->tau_gaba[i_BRAHMS] = (RNOR(&this->rngData_BRAHMS) * val2_BRAHMS) + val1_BRAHMS;
					}
				}
				// if uniform distribution
				if (this->tau_gaba[0] == 2) {
					val1_BRAHMS = this->tau_gaba[1];
					val2_BRAHMS = this->tau_gaba[2];
					this->rngData_BRAHMS.seed = this->tau_gaba[3];
					if (this->rngData_BRAHMS.seed == 0) {
						this->rngData_BRAHMS.seed = getTime();
					}
					this->tau_gaba.resize(numEl_BRAHMS, 0);
					for (UINT32 i_BRAHMS = 0; i_BRAHMS < numEl_BRAHMS; ++i_BRAHMS) {
						this->tau_gaba[i_BRAHMS] = (UNI(&this->rngData_BRAHMS) * (val2_BRAHMS-val1_BRAHMS)) + val1_BRAHMS;
					}
				}
				finishedThis = true;
			}
			if (nodeState.hasField("tau_gabaOVER2") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				if (nodeState.getField("tau_gabaOVER2").getDims()[0] == 1) {
					this->tau_gaba = nodeState.getField("tau_gabaOVER2").getArrayDOUBLE();
					if (this->tau_gaba.size() == 1) {
						this->tau_gaba.resize(numEl_BRAHMS, tau_gaba[0]);
					} else if (this->tau_gaba.size() != numEl_BRAHMS) {
						berr << "Parameter tau_gaba has incorrect dimensions (Its size is " << this->tau_gaba.size() << ", not " << numEl_BRAHMS << ")";
					}
					finishedThis = true;
				}
			}

			if (nodeState.hasField("tau_gabaRANDX") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				this->tau_gaba = nodeState.getField("tau_gabaRANDX").getArrayDOUBLE();
				float val1_BRAHMS = 0;
				float val2_BRAHMS = 1;
				// if normal distribution
				if (this->tau_gaba[0] == 1) {
			...
			if (nodeState.hasField("tau_gaba") && !finishedThis)
			{
				this->tau_gaba = nodeState.getField("tau_gaba").getArrayDOUBLE();
				if (this->tau_gaba.size() == 1) {
					this->tau_gaba.resize(numEl_BRAHMS, tau_gaba[0]);
				} else if (this->tau_gaba.size() != numEl_BRAHMS) {
					berr << "Parameter tau_gaba has incorrect dimensions (Its size is " << tau_gaba.size() << ", not " << numEl_BRAHMS << ")";
				}
			}

			if (nodeState.hasField("tau_gabaBIN_FILE_NAME") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
			        if (!nodeState.hasField("tau_gabaBIN_NUM_ELEMENTS")) {
				        berr << "Found a binary file name without corresponding binary number of elements.";
				}
				string tau_gaba_BINARY_FILE_NAME = nodeState.getField("tau_gabaBIN_FILE_NAME").getSTRING();
				int __temp_num_property_elements = nodeState.getField("tau_gabaBIN_NUM_ELEMENTS").getINT32();

				// open the file for reading
				FILE * binfile;

				string fileName = modelDirectory_BRAHMS + "/" + tau_gaba_BINARY_FILE_NAME;
				binfile = fopen(fileName.c_str(),"rb");
				if (!binfile) {
					berr << "Could not open properties list file: " << fileName;
				}

				this->tau_gaba.resize(__temp_num_property_elements);
				vector<INT32> __temp_property_list_indices (__temp_num_property_elements, 0);
				vector<DOUBLE> __temp_property_list_values (__temp_num_property_elements, 0);
				for (int i_BRAHMS = 0; i_BRAHMS < __temp_num_property_elements; ++i_BRAHMS) {
				...
				for (int i_BRAHMS = 0; i_BRAHMS < __temp_num_property_elements; ++i_BRAHMS) {
				        if ( __temp_property_list_indices[i_BRAHMS] > __temp_num_property_elements
					    || __temp_property_list_indices[i_BRAHMS] < 0) {
					    berr << "Error loading parameter binary property tau_gaba: index "
						 << __temp_property_list_indices[i_BRAHMS] << " out of range";
					}
					this->tau_gaba[__temp_property_list_indices[i_BRAHMS]] = __temp_property_list_values[i_BRAHMS];
				}
			}

			if (nodeState.hasField("tau_gabaOVER1") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				vector < double > __tempInputValues__;
				__tempInputValues__ = nodeState.getField("tau_gabaOVER1").getArrayDOUBLE();
				// since OVER1 always means sparse Values
				if (this->tau_gaba.size() == 1) {
					this->tau_gaba.resize(numEl_BRAHMS, tau_gaba[0]);
				}
				if (this->tau_gaba.size() != numEl_BRAHMS) {
					berr << "Parameter tau_gaba has a ValueList override but no base Values";
				}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS < __tempInputValues__.size(); i_BRAHMS += 2) {
					if (__tempInputValues__[i_BRAHMS] > numEl_BRAHMS-1) {
						berr << "Parameter tau_gaba has ValueList index " << float(i_BRAHMS)<< " (value of " << float(__tempInputValues__[i_BRAHMS]) << ") out of range";
					}
					this->tau_gaba[__tempInputValues__[i_BRAHMS]] = __tempInputValues__[i_BRAHMS+1];
				}
			}
			...
			if (nodeState.hasField("tau_gabaOVER2") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				vector < double > __tempInputValues__;
				__tempInputValues__ = nodeState.getField("tau_gabaOVER2").getArrayDOUBLE();
				// OVER2 here means sparse Values
				if (this->tau_gaba.size() != numEl_BRAHMS) {
					berr << "Experiment Parameter tau_gaba has a ValueList override but no base Values";
				}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS < __tempInputValues__.size(); i_BRAHMS += 2) {
					if (__tempInputValues__[i_BRAHMS] > numEl_BRAHMS-1) {
						berr << "Experiment Parameter tau_gaba has ValueList index " << float(i_BRAHMS)<< " (value of " << float(__tempInputValues__[i_BRAHMS]) << ") out of range";
					}
					this->tau_gaba[__tempInputValues__[i_BRAHMS]] = __tempInputValues__[i_BRAHMS+1];
				}
			}
			// output the values:
			/*bout << "tau_gaba" << D_WARN;
			for (UINT32 i_BRAHMS = 0; i_BRAHMS < tau_gaba.size(); ++i_BRAHMS) {
				bout << float(tau_gaba[i_BRAHMS]) << D_WARN;
			}*/
			}

			{
			bool finishedThis = false;
			if (nodeState.hasField("E_gabaRANDXOVER2")) {
				this->allParamsDelaysAreFixedValue = false;
				this->E_gaba = nodeState.getField("E_gabaRANDXOVER2").getArrayDOUBLE();
				float val1_BRAHMS = 0;
				float val2_BRAHMS = 1;
				// if normal distribution
				if (this->E_gaba[0] == 1) {
					val1_BRAHMS = this->E_gaba[1];
					val2_BRAHMS = this->E_gaba[2];
					this->rngData_BRAHMS.seed = E_gaba[3];
					if (this->rngData_BRAHMS.seed == 0) {
						this->rngData_BRAHMS.seed = getTime();
					}
					this->E_gaba.resize(numEl_BRAHMS, 0);
					for (UINT32 i_BRAHMS = 0; i_BRAHMS < numEl_BRAHMS; ++i_BRAHMS) {
						this->E_gaba[i_BRAHMS] = (RNOR(&this->rngData_BRAHMS) * val2_BRAHMS) + val1_BRAHMS;
					}
				}
				// if uniform distribution
				if (this->E_gaba[0] == 2) {
					val1_BRAHMS = this->E_gaba[1];
					val2_BRAHMS = this->E_gaba[2];
					this->rngData_BRAHMS.seed = this->E_gaba[3];
					if (this->rngData_BRAHMS.seed == 0) {
						this->rngData_BRAHMS.seed = getTime();
					}
					this->E_gaba.resize(numEl_BRAHMS, 0);
					for (UINT32 i_BRAHMS = 0; i_BRAHMS < numEl_BRAHMS; ++i_BRAHMS) {
						this->E_gaba[i_BRAHMS] = (UNI(&this->rngData_BRAHMS) * (val2_BRAHMS-val1_BRAHMS)) + val1_BRAHMS;
					}
				}
				finishedThis = true;
			}
			if (nodeState.hasField("E_gabaOVER2") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				if (nodeState.getField("E_gabaOVER2").getDims()[0] == 1) {
					this->E_gaba = nodeState.getField("E_gabaOVER2").getArrayDOUBLE();
					if (this->E_gaba.size() == 1) {
						this->E_gaba.resize(numEl_BRAHMS, E_gaba[0]);
					} else if (this->E_gaba.size() != numEl_BRAHMS) {
						berr << "Parameter E_gaba has incorrect dimensions (Its size is " << this->E_gaba.size() << ", not " << numEl_BRAHMS << ")";
					}
					finishedThis = true;
				}
			}

			if (nodeState.hasField("E_gabaRANDX") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				this->E_gaba = nodeState.getField("E_gabaRANDX").getArrayDOUBLE();
				float val1_BRAHMS = 0;
				float val2_BRAHMS = 1;
				// if normal distribution
				if (this->E_gaba[0] == 1) {
			...
			if (nodeState.hasField("E_gaba") && !finishedThis)
			{
				this->E_gaba = nodeState.getField("E_gaba").getArrayDOUBLE();
				if (this->E_gaba.size() == 1) {
					this->E_gaba.resize(numEl_BRAHMS, E_gaba[0]);
				} else if (this->E_gaba.size() != numEl_BRAHMS) {
					berr << "Parameter E_gaba has incorrect dimensions (Its size is " << E_gaba.size() << ", not " << numEl_BRAHMS << ")";
				}
			}

			if (nodeState.hasField("E_gabaBIN_FILE_NAME") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
			        if (!nodeState.hasField("E_gabaBIN_NUM_ELEMENTS")) {
				        berr << "Found a binary file name without corresponding binary number of elements.";
				}
				string E_gaba_BINARY_FILE_NAME = nodeState.getField("E_gabaBIN_FILE_NAME").getSTRING();
				int __temp_num_property_elements = nodeState.getField("E_gabaBIN_NUM_ELEMENTS").getINT32();

				// open the file for reading
				FILE * binfile;

				string fileName = modelDirectory_BRAHMS + "/" + E_gaba_BINARY_FILE_NAME;
				binfile = fopen(fileName.c_str(),"rb");
				if (!binfile) {
					berr << "Could not open properties list file: " << fileName;
				}

				this->E_gaba.resize(__temp_num_property_elements);
				vector<INT32> __temp_property_list_indices (__temp_num_property_elements, 0);
				vector<DOUBLE> __temp_property_list_values (__temp_num_property_elements, 0);
				for (int i_BRAHMS = 0; i_BRAHMS < __temp_num_property_elements; ++i_BRAHMS) {
				...
				for (int i_BRAHMS = 0; i_BRAHMS < __temp_num_property_elements; ++i_BRAHMS) {
				        if ( __temp_property_list_indices[i_BRAHMS] > __temp_num_property_elements
					    || __temp_property_list_indices[i_BRAHMS] < 0) {
					    berr << "Error loading parameter binary property E_gaba: index "
						 << __temp_property_list_indices[i_BRAHMS] << " out of range";
					}
					this->E_gaba[__temp_property_list_indices[i_BRAHMS]] = __temp_property_list_values[i_BRAHMS];
				}
			}

			if (nodeState.hasField("E_gabaOVER1") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				vector < double > __tempInputValues__;
				__tempInputValues__ = nodeState.getField("E_gabaOVER1").getArrayDOUBLE();
				// since OVER1 always means sparse Values
				if (this->E_gaba.size() == 1) {
					this->E_gaba.resize(numEl_BRAHMS, E_gaba[0]);
				}
				if (this->E_gaba.size() != numEl_BRAHMS) {
					berr << "Parameter E_gaba has a ValueList override but no base Values";
				}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS < __tempInputValues__.size(); i_BRAHMS += 2) {
					if (__tempInputValues__[i_BRAHMS] > numEl_BRAHMS-1) {
						berr << "Parameter E_gaba has ValueList index " << float(i_BRAHMS)<< " (value of " << float(__tempInputValues__[i_BRAHMS]) << ") out of range";
					}
					this->E_gaba[__tempInputValues__[i_BRAHMS]] = __tempInputValues__[i_BRAHMS+1];
				}
			}
			...
			if (nodeState.hasField("E_gabaOVER2") && !finishedThis) {
				this->allParamsDelaysAreFixedValue = false;
				vector < double > __tempInputValues__;
				__tempInputValues__ = nodeState.getField("E_gabaOVER2").getArrayDOUBLE();
				// OVER2 here means sparse Values
				if (this->E_gaba.size() != numEl_BRAHMS) {
					berr << "Experiment Parameter E_gaba has a ValueList override but no base Values";
				}
				for (UINT32 i_BRAHMS = 0; i_BRAHMS < __tempInputValues__.size(); i_BRAHMS += 2) {
					if (__tempInputValues__[i_BRAHMS] > numEl_BRAHMS-1) {
						berr << "Experiment Parameter E_gaba has ValueList index " << float(i_BRAHMS)<< " (value of " << float(__tempInputValues__[i_BRAHMS]) << ") out of range";
					}
					this->E_gaba[__tempInputValues__[i_BRAHMS]] = __tempInputValues__[i_BRAHMS+1];
				}
			}
			// output the values:
			/*bout << "E_gaba" << D_WARN;
			for (UINT32 i_BRAHMS = 0; i_BRAHMS < E_gaba.size(); ++i_BRAHMS) {
				bout << float(E_gaba[i_BRAHMS]) << D_WARN;
			}*/
			}
			...
			// Alias resize
I_gaba.resize(numEl_BRAHMS, 0);


			// Log base name
			baseNameForLogs_BRAHMS = "../log/" + nodeState.getField("logfileNameForComponent").getSTRING();
			g_gaba_BINARY_FILE_NAME_OUT = "../model/" + nodeState.getField("g_gabaBIN_FILE_NAME").getSTRING();
			...
			if (nodeState.hasField("I_gabaLOG")) {
				// we have a log! Read the data in:
				// check we got some data first
				VDOUBLE tempLogData_BRAHMS = nodeState.getField("I_gabaLOG").getArrayDOUBLE();
				if (tempLogData_BRAHMS.size() == 0) berr << "ERROR: log with no indices";
				// if we are logging 'all'
				if (tempLogData_BRAHMS[0] < -0.1) {
					I_gabaLOGMAP.push_back(-2);
				} else {
				// otherwise
					// resize the logvar:
					I_gabaLOGVAR.resize(tempLogData_BRAHMS.size(),0);
					// resize the logmap:
					I_gabaLOGMAP.resize(numEl_BRAHMS,-1);
					...
					for (unsigned int i_BRAHMS = 0; i_BRAHMS < tempLogData_BRAHMS.size(); ++i_BRAHMS) {
						if (tempLogData_BRAHMS[i_BRAHMS]+0.5 > numEl_BRAHMS) {
							...
							I_gabaLOGMAP[(int) tempLogData_BRAHMS[i_BRAHMS]] = i_BRAHMS;
						}
					}
				}
				// open the logfile for writing
				string logFileName_BRAHMS = baseNameForLogs_BRAHMS;
				logFileName_BRAHMS.append("_I_gaba_log.bin");
				I_gabaLOGFILE = fopen(logFileName_BRAHMS.c_str(),"wb");
			}

			// check for existence of log stateNode
			if (nodeState.hasField("g_gabaLOG")) {
				// we have a log! Read the data in:
				// check we got some data first
				VDOUBLE tempLogData_BRAHMS = nodeState.getField("g_gabaLOG").getArrayDOUBLE();
				if (tempLogData_BRAHMS.size() == 0) berr << "ERROR: log with no indices";
				// if we are logging 'all'
				if (tempLogData_BRAHMS[0] < -0.1) {
					g_gabaLOGMAP.push_back(-2);
				} else {
				// otherwise
					// resize the logvar:
					g_gabaLOGVAR.resize(tempLogData_BRAHMS.size(),0);
					// resize the logmap:
					g_gabaLOGMAP.resize(numEl_BRAHMS,-1);
					// set the logmap values - checking for out of range values
					for (unsigned int i_BRAHMS = 0; i_BRAHMS < tempLogData_BRAHMS.size(); ++i_BRAHMS) {
						if (tempLogData_BRAHMS[i_BRAHMS]+0.5 > numEl_BRAHMS) {
							bout << "Attempting to log an index out of range" << D_WARN;
							...
							// set in mapping that the i_BRAHMSth log value relates to the tempLogData_BRAHMS[i_BRAHMS]th neuron
							g_gabaLOGMAP[(int) tempLogData_BRAHMS[i_BRAHMS]] = i_BRAHMS;
						}
					}
				}
				// open the logfile for writing
				string logFileName_BRAHMS = baseNameForLogs_BRAHMS;
				logFileName_BRAHMS.append("_g_gaba_log.bin");
				g_gabaLOGFILE = fopen(logFileName_BRAHMS.c_str(),"wb");
			}

            GABAbufO__Oregime.resize(numEl_BRAHMS,1);
            GABAbufO__OregimeNext.resize(numEl_BRAHMS,0);

			dt = 1000.0f * time->sampleRate.den / time->sampleRate.num;


(CurrentP__Pg_gaba) = &COMPONENT_CLASS_CPP::CurrentV__Vg_gaba;
		...
		case EVENT_INIT_CONNECT:
		{
			...
			if (event->flags & F_FIRST_CALL)
			{
				...
				PORTI_gaba.setName("I_gaba");
				PORTI_gaba.create(hComponent);
				PORTI_gaba.setStructure(TYPE_REAL | TYPE_DOUBLE, Dims(numElements_BRAHMS).cdims());

				PORTg_gaba.setName("g_gaba");
				PORTg_gaba.create(hComponent);
				PORTg_gaba.setStructure(TYPE_REAL | TYPE_DOUBLE, Dims(numElements_BRAHMS).cdims());
		...
		case EVENT_RUN_SERVICE:
		{
            ...
            for (int i_BRAHMS = 0; i_BRAHMS < GABAbufO__Oregime.size(); ++i_BRAHMS) {

                GABAbufO__OregimeNext[i_BRAHMS] = GABAbufO__Oregime[i_BRAHMS];
		...
		DOUBLE* DATAv;
			...
			for (int in_BRAHMS = 0; in_BRAHMS < DATAS.size(); ++in_BRAHMS) {
				...
				S[num_BRAHMS] = impulseValue__In;
					...
					switch (GABAbufO__Oregime[num_BRAHMS]) {

						//Regime
						case GABAbufX__XCurrent:

					//OnImpulse1
					GABAbufO__OregimeNext[num_BRAHMS]=GABAbufX__XCurrent;
			...
			for (num_BRAHMS = 0; num_BRAHMS < numEl_BRAHMS; ++num_BRAHMS) {
				...
				switch (GABAbufO__Oregime[num_BRAHMS]) {
					...
					//TimeDerivative
					float g_gaba_tempVALforINT = this->integrate(float(g_gaba[num_BRAHMS]), CurrentP__Pg_gaba , num_BRAHMS);

					//TimeDerivative finalisation
					g_gaba[num_BRAHMS] = g_gaba_tempVALforINT;
			...
			for (num_BRAHMS = 0; num_BRAHMS < numEl_BRAHMS; ++num_BRAHMS) {
                            if (buffer[num_BRAHMS] <= buffer_max[num_BRAHMS]) {
                                g_gaba[num_BRAHMS] = g_gaba[num_BRAHMS] + g_bar[num_BRAHMS]*buffer[num_BRAHMS];
                                buffer[num_BRAHMS] = 0;
                            } else {
                                g_gaba[num_BRAHMS] = g_gaba[num_BRAHMS] + g_bar[num_BRAHMS]*buffer_max[num_BRAHMS];
			...
			for (num_BRAHMS = 0; num_BRAHMS < numEl_BRAHMS; ++num_BRAHMS) {
					...
					I_gaba[num_BRAHMS]= g_gaba[num_BRAHMS]*(v[num_BRAHMS]-E_gaba[num_BRAHMS]);
            ...
            for (int i_BRAHMS = 0; i_BRAHMS < GABAbufO__Oregime.size(); ++i_BRAHMS) {
                GABAbufO__Oregime[i_BRAHMS] = GABAbufO__OregimeNext[i_BRAHMS];

                // updating logs...

				if (I_gabaLOGMAP.size() > 0) {
					if (I_gabaLOGMAP[0] > -1.1) {
						if (I_gabaLOGMAP[i_BRAHMS]+1)
							I_gabaLOGVAR[I_gabaLOGMAP[i_BRAHMS]] = I_gaba[i_BRAHMS];
					}
				}

				if (g_gabaLOGMAP.size() > 0) {
					if (g_gabaLOGMAP[0] > -1.1) {
						if (g_gabaLOGMAP[i_BRAHMS]+1)
							g_gabaLOGVAR[g_gabaLOGMAP[i_BRAHMS]] = g_gaba[i_BRAHMS];
					}
				}
			...
			// writing logs...
				...
				if (I_gabaLOGMAP.size() > 0) {
					if (I_gabaLOGMAP[0] < -1.1) {
						// write data
						size_t written_BRAHMS = fwrite(&I_gaba[0],sizeof(double),I_gaba.size(),I_gabaLOGFILE);
						if (written_BRAHMS != I_gaba.size()) berr << "Error writing logfile for " << baseNameForLogs_BRAHMS << "_I_gaba";
					} else {
						// write data
						size_t written_BRAHMS = fwrite(&I_gabaLOGVAR[0],sizeof(double),I_gabaLOGVAR.size(),I_gabaLOGFILE);
						if (written_BRAHMS != I_gabaLOGVAR.size()) berr << "Error writing logfile for " << baseNameForLogs_BRAHMS << "_I_gaba";
					}
				}
				...
				if (g_gabaLOGMAP.size() > 0) {
					if (g_gabaLOGMAP[0] < -1.1) {
						// write data
						size_t written_BRAHMS = fwrite(&g_gaba[0],sizeof(double),g_gaba.size(),g_gabaLOGFILE);
						if (written_BRAHMS != g_gaba.size()) berr << "Error writing logfile for " << baseNameForLogs_BRAHMS << "_g_gaba";
					} else {
						// write data
						size_t written_BRAHMS = fwrite(&g_gabaLOGVAR[0],sizeof(double),g_gabaLOGVAR.size(),g_gabaLOGFILE);
						if (written_BRAHMS != g_gabaLOGVAR.size()) berr << "Error writing logfile for " << baseNameForLogs_BRAHMS << "_g_gaba";
					}
				}

			PORTI_gaba.setContent(&I_gaba[0]);

			PORTg_gaba.setContent(&g_gaba[0]);
		...
		case EVENT_RUN_STOP:
		{
			...
			if (I_gabaLOGMAP.size() > 0) {
				...
				FILE * I_gabaLOGREPORT;
				string logFileName_BRAHMS = baseNameForLogs_BRAHMS;
				logFileName_BRAHMS.append("_I_gaba_logrep.xml");
				I_gabaLOGREPORT = fopen(logFileName_BRAHMS.c_str(),"w");
  			...
  			logFileName_BRAHMS = logFileName_BRAHMS.substr(found+1);
				fprintf(I_gabaLOGREPORT, "<LogReport>\n");
				fprintf(I_gabaLOGREPORT, "	<AnalogLog>\n");
				fprintf(I_gabaLOGREPORT, "		<LogFile>%s</LogFile>\n",logFileName_BRAHMS.c_str());
				fprintf(I_gabaLOGREPORT, "		<LogFileType>binary</LogFileType>\n");
				fprintf(I_gabaLOGREPORT, "		<LogEndTime>%f</LogEndTime>\n",t);
				if (I_gabaLOGMAP[0] > -1.1) {
					for (unsigned int i = 0; i < I_gabaLOGMAP.size(); ++i) {
						if (I_gabaLOGMAP[i] > -0.1) {
							fprintf(I_gabaLOGREPORT, "		<LogCol index=\"%d\" heading=\"I_gaba\" dims=\"\" type=\"double\"/>\n",i);
						}
					}
				} else {
					fprintf(I_gabaLOGREPORT, "		<LogAll size=\"%d\" headings=\"I_gaba\" type=\"double\" dims=\"\"/>\n",numEl_BRAHMS);
				}
				fprintf(I_gabaLOGREPORT,"		<TimeStep dt=\"%f\"/>\n", dt);
				fprintf(I_gabaLOGREPORT, "	</AnalogLog>\n");
				fprintf(I_gabaLOGREPORT, "</LogReport>\n");

				fclose(I_gabaLOGREPORT);
				fclose(I_gabaLOGFILE);
			}

			if (g_gabaLOGMAP.size() > 0) {

				FILE * g_gabaLOGREPORT;
				string logFileName_BRAHMS = baseNameForLogs_BRAHMS;
				logFileName_BRAHMS.append("_g_gaba_logrep.xml");
				g_gabaLOGREPORT = fopen(logFileName_BRAHMS.c_str(),"w");
				logFileName_BRAHMS = baseNameForLogs_BRAHMS;
				logFileName_BRAHMS.append("_g_gaba_log.bin");
  			unsigned found = logFileName_BRAHMS.find_last_of("/\\");
  			logFileName_BRAHMS = logFileName_BRAHMS.substr(found+1);
				fprintf(g_gabaLOGREPORT, "<LogReport>\n");
				fprintf(g_gabaLOGREPORT, "	<AnalogLog>\n");
				fprintf(g_gabaLOGREPORT, "		<LogFile>%s</LogFile>\n",logFileName_BRAHMS.c_str());
				fprintf(g_gabaLOGREPORT, "		<LogFileType>binary</LogFileType>\n");
				fprintf(g_gabaLOGREPORT, "		<LogEndTime>%f</LogEndTime>\n",t);
				if (g_gabaLOGMAP[0] > -1.1) {
					for (unsigned int i = 0; i < g_gabaLOGMAP.size(); ++i) {
						if (g_gabaLOGMAP[i] > -0.1) {
							fprintf(g_gabaLOGREPORT, "		<LogCol index=\"%d\" heading=\"g_gaba\" dims=\"\" type=\"double\"/>\n",i);
						}
					}
				} else {
					fprintf(g_gabaLOGREPORT, "		<LogAll size=\"%d\" headings=\"g_gaba\" type=\"double\" dims=\"\"/>\n",numEl_BRAHMS);
				}
				fprintf(g_gabaLOGREPORT,"		<TimeStep dt=\"%f\"/>\n", dt);
				fprintf(g_gabaLOGREPORT, "	</AnalogLog>\n");
				fprintf(g_gabaLOGREPORT, "</LogReport>\n");

				fclose(g_gabaLOGREPORT);
				fclose(g_gabaLOGFILE);
			}

			// Write variable name: g_gaba into a file.
			{
				unsigned char copy_buffer_BRAHMS[1024] = "";
				FILE* g_gaba_svfile;
				string g_gaba_fileName = g_gaba_BINARY_FILE_NAME_OUT;
#ifdef MAKE_BACKUP_OF_ORIGINAL_STATE
				g_gaba_svfile = fopen (g_gaba_fileName.c_str(), "rb");
				if (g_gaba_svfile) {

					string g_gaba_fileName_backup = g_gaba_fileName + ".bu";
					FILE* g_gaba_svfile_backup;
					g_gaba_svfile_backup = fopen (g_gaba_fileName_backup.c_str(), "wb");
					if (!g_gaba_svfile) {
						berr << "Could not open state variable backup file: " << g_gaba_fileName_backup;
					}

					size_t bytes_copied_BRAHMS = 0;
					size_t bytes_really_copied = 0;
					while ((bytes_copied_BRAHMS = fread (copy_buffer_BRAHMS, 1, 1024, g_gaba_svfile)) > 0) {
						bytes_really_copied = fwrite (copy_buffer_BRAHMS, 1, bytes_copied_BRAHMS, g_gaba_svfile_backup);
						if (bytes_really_copied != bytes_copied_BRAHMS) {
							berr << "Error making state variable backup file: " << g_gaba_fileName_backup;
						}
					}
					fclose (g_gaba_svfile_backup);
					fclose (g_gaba_svfile);
				}
#endif
				g_gaba_svfile = fopen (g_gaba_fileName.c_str(), "wb");
				if (!g_gaba_svfile) {
					berr << "Could not open state variable file: " << g_gaba_fileName;
				}

				int writertn_BRAHMS = 0, writeiter_BRAHMS = 0;
				while (writeiter_BRAHMS < this->g_gaba.size()) {
					writertn_BRAHMS = fwrite (reinterpret_cast<const char*>(&writeiter_BRAHMS), sizeof(int), 1, g_gaba_svfile);
					if (writertn_BRAHMS != 1) {
						berr << "Failed to write index into " << g_gaba_fileName << ". Wrote " << writertn_BRAHMS << " ints, rather than 1";
					}
					writertn_BRAHMS = fwrite (reinterpret_cast<const char*>(&this->g_gaba[writeiter_BRAHMS]), sizeof(double), 1, g_gaba_svfile);
					if (writertn_BRAHMS != 1) {
						berr << "Failed to write data into " << g_gaba_fileName << ". Wrote " << writertn_BRAHMS << " doubles, rather than 1";
					}
					++writeiter_BRAHMS;
				}
